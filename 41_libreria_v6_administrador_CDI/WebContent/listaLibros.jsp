...
<br/><br/>
				<h1>Carrito </h1>
		<c:if test="${!empty sessionScope.clienteAutenticado.carrito}">
			<table border="1">
			    <tr><th>Titulo</th><th>Autor</th><th>Precio</th><th></th></tr>		    	
			    	<c:forEach var="carro" items="${sessionScope.clienteAutenticado.carrito}" varStatus="s">
			    		<tr>
			    			<td>${carro.titulo}</td>
							<td>${carro.autor}</td>
							<td>${carro.precio}</td>
			...
			<a href="Controller?temas=${param.temas}&option=doProcesarCompra">Procesar Compra</a>
		<br/><br/>
